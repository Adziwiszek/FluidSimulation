...
void FluidGrid::placeSolid(float x, float y, float radius) {
  int left_bound = utils::max<int>(x - radius, 1.0f);
  int right_bound = utils::max<int>(x + radius, N[0] - 1);
  int top_bound = utils::max<int>(y + radius, N[1] - 1);
  int down_bound = utils::max<int>(y - radius, 1.0f);

  for(int row = left_bound; row <= right_bound; row++) {
    for(int col = down_bound; col <= top_bound; col++) {
...
void FluidGrid::placeFluid(float x, float y, float radius) {
  int left_bound = utils::max<int>(x - radius, 1.0f);
  int right_bound = utils::min<int>(x + radius, N[0] - 1);
  int top_bound = utils::min<int>(y + radius, N[1] - 1);
  int down_bound = utils::max<int>(y - radius, 1.0f);
